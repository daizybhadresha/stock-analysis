...
class StockTrade:
    ...
    def __init__(self):
        self._trade_data = pd.DataFrame(columns=[self.T_TIMESTAMP_COL,self.STOCK_SYMBOL_COL,self.T_INDICATOR_COL,self.T_QUANTITY_COL,self.T_TRADED_PRICE_COL])
    ...
    def record_trade(timestamp: str, stock_symbol: str, trans_type: str, volume: float, price: float):
        '''
        ...
        if time == None:
            ...
            self._trade_data[self.T_TIMESTAMP_COL] = current_time
        
        self._trade_data[self.T_TIMESTAMP_COL] = timestamp
        self._trade_data[self.STOCK_SYMBOL_COL] = stock_symbol
        self._trade_data[self.T_INDICATOR_COL] = trans_type
        self._trade_data[self.T_TRADED_PRICE_COL] = price
    ...
    def calculate_dividend(self, stock_symbol, stock_price):
        ...
        if stock_type == const.self.TYPE_COMMON:
            dividend = last_dividend/stock_price
        elif stock_type == const.self.TYPE_PREFERRED:
